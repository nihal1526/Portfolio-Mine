...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const CHATBOT_ENDPOINT = "https://portfolio1526.app.n8n.cloud/webhook/ai/agent/main";
        ...
        if (CHATBOT_ENDPOINT) {
            fetch(CHATBOT_ENDPOINT, {
                ...
                .then(text => {
                    ...
                    if (!text || text.trim() === "") {
                        // Helpful diagnostic message for when webhook returns an empty string (Response Mode set incorrectly in n8n)
                        replyText = "⚠️ **DIAGNOSTIC NOTICE**: The chatbot webhook responded successfully (HTTP 200) but returned an **empty response body**.\n\nTo fix this in your n8n workspace:\n1. Open your chatbot n8n workflow.\n2. Open the **Webhook Trigger Node**.\n3. Change **Response Mode** to **'When Last Node Finishes'** (or add a **'Respond to Webhook'** node at the end returning the AI Agent's output).\n\nIf you prefer to contact Nihal directly:\n📞 PHONE: +971 554491295\n📧 EMAIL: nihalsjain152002@gmail.com";
                        typingBubble.remove();
